...
const LeftSidebar: React.FC<LeftSidebarProps> = ({ isExpanded, setIsExpanded, username, onOpenAboutModal }) => {
  ...
  return (
    <aside
      // --- CHANGES ARE HERE ---
      className={`fixed top-24 left-0 h-[calc(100vh-theme(space.24))] bg-gray-100 dark:bg-secondary border-r border-tertiary-light dark:border-tertiary z-30 transition-all duration-300 ease-in-out ${
      ...
      <div className="flex flex-col h-full p-3 pt-6">
        ...
        <div className="relative mt-auto group">
          <div className="absolute bottom-full left-0 w-full mb-2 bg-secondary-light dark:bg-primary border border-tertiary-light dark:border-tertiary rounded-lg shadow-lg py-1 transition-all duration-200 opacity-0 invisible group-hover:opacity-100 group-hover:visible">
            {profile?.username && (
                <Link to={`/profile/${profile.username}`} className="flex items-center gap-3 px-4 py-2 text-sm text-text-secondary-light dark:text-text-secondary hover:bg-tertiary-light dark:hover:bg-tertiary">
            ...
            <button onClick={handleSignOut} className="w-full text-left flex items-center gap-3 px-4 py-2 text-sm text-red-500 hover:bg-tertiary-light dark:hover:bg-tertiary">
          ...
          <div
            className="flex items-center w-full p-2 space-x-3 rounded-lg hover:bg-tertiary-light dark:hover:bg-tertiary transition-colors cursor-pointer"
          >
            <img src={profile?.avatar_url || ''} alt="profile" className="w-9 h-9 rounded-full object-cover flex-shrink-0" />
            <div className={`flex-1 text-left min-w-0 transition-opacity duration-200 ${isExpanded ? 'opacity-100' : 'opacity-0'}`}>
              <p className="font-bold text-sm text-text-main-light dark:text-text-main truncate">{profile?.full_name}</p>
