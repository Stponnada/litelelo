    ...
    return (
        <div className="min-h-screen bg-primary flex flex-col items-center justify-center p-4 font-sans">
            <div className="absolute top-4 left-4">
                <Link href="/" className="text-text-tertiary hover:text-brand-green transition-colors">
                ...
                >
                    {Array.from({ length: BOARD_SIZE * BOARD_SIZE }).map((_, i) => {
                        ...
                        return (
                            <div
                                key={i}
                                className={`w-full h-full ${isSnake ? 'bg-brand-green rounded-sm' : isFood ? 'bg-red-500 rounded-full' : 'bg-primary'
                                    }`}
                            />
                        );
                    })}
