...
const EventDetailPage: React.FC = () => {
    ...
    return (
        <div className="max-w-4xl mx-auto px-4 md:px-0 py-4 md:py-8">
            {isEditModalOpen && (
                <CreateEventModal
            ...
            <Link href="/campus/events" className="text-sm text-text-secondary-light dark:text-text-secondary hover:underline mb-4 inline-block">
            ...
            <div className="bg-secondary-light dark:bg-secondary rounded-xl shadow-lg border border-tertiary-light dark:border-tertiary overflow-hidden">
                <div className="w-full h-48 md:h-64 bg-tertiary-light dark:bg-tertiary">
                    {event.image_url && <img src={event.image_url} alt={event.name} className="w-full h-full object-cover" />}
                </div>
                <div className="p-4 md:p-6">
                    <h1 className="text-2xl md:text-4xl font-bold text-text-main-light dark:text-text-main">{event.name}</h1>
                    <div className="flex items-center space-x-2 mt-4 text-text-secondary-light dark:text-text-secondary">
                        ...
                        <span>Hosted by <Link href={`/profile/${event.created_by.username}`} className="font-semibold hover:underline">{event.created_by.full_name}</Link></span>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-3 mt-6">
                        ...
                        {isOwner && (
                            <div className="ml-auto flex items-center gap-2 w-full md:w-auto justify-end mt-2 md:mt-0">
                                <button onClick={() => setIsEditModalOpen(true)} className="font-semibold py-2 px-4 rounded-full border border-tertiary-light dark:border-tertiary hover:bg-tertiary-light dark:hover:bg-tertiary text-sm md:text-base">Edit</button>
                                <button onClick={handleDelete} disabled={isDeleting} className="font-semibold py-2 px-4 rounded-full border border-red-500/50 text-red-500 hover:bg-red-500 hover:text-white disabled:opacity-50 text-sm md:text-base">
            ...
            <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                <div className="bg-secondary-light dark:bg-secondary p-4 md:p-6 rounded-xl border border-tertiary-light dark:border-tertiary">
                    <h2 className="text-xl md:text-2xl font-bold mb-4">Attendees ({goingRsvps.length})</h2>
                    <div className="flex flex-wrap gap-2">
                        {goingRsvps.map(rsvp => (
                            <Link href={`/profile/${rsvp.profiles.username}`} key={rsvp.user_id} title={rsvp.profiles.full_name || rsvp.profiles.username}>
                                <img src={rsvp.profiles.avatar_url || ''} alt={rsvp.profiles.username} className="w-10 h-10 md:w-12 md:h-12 rounded-full object-cover ring-2 ring-secondary-light dark:ring-secondary" />
                            </Link>
                        ))}
                    </div>
                </div>
