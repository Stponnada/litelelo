...
const MarketplaceItemPage: React.FC = () => {
    ...
    const handleContactSeller = () => {
        if (!listing) return;
        router.push(`/chat?recipient=${listing.seller_id.user_id}`);
    };
    ...
    if (error) {
        return (
            <div className="p-8 text-center text-red-500">
                ...
                <p>{error}</p>
                <Link href="/campus/marketplace" className="mt-4 inline-block px-6 py-2 bg-brand-green text-black font-bold rounded-lg">
                    Back to Marketplace
                </Link>
            </div>
        );
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <button onClick={() => router.back()} className="mb-6 flex items-center gap-2 text-sm font-semibold text-text-secondary-light dark:text-text-secondary hover:text-brand-green transition-colors">
