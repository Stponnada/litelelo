...
import { Conversation as ConversationType, Profile } from '@/types';
...
const GroupInfoPage: React.FC = () => {
    ...
    const [groupInfo, setGroupInfo] = useState<Partial<ConversationType> | null>(null);
        ...
        const fetchGroupInfo = async () => {
            ...
            try {
                ...
                const { data: conversationData, error: convError } = await supabase
                    ...
                    .single();
                if (convError || !conversationData || conversationData.type !== 'group') throw new Error("Group not found.");
    ...
    return (
        <div className="max-w-4xl mx-auto">
            <header className="flex items-center p-2 mb-6">
                <button onClick={() => router.back()} className="p-2 text-text-secondary-light dark:text-gray-300 rounded-full hover:bg-tertiary-light dark:hover:bg-tertiary">
                    <BackIcon className="w-6 h-6" />
                </button>
                <h1 className="text-2xl font-bold ml-4 text-text-main-light dark:text-text-main">{groupInfo?.name || 'Group Info'}</h1>
            </header>
            ...
            <div className="bg-secondary-light dark:bg-secondary p-6 rounded-lg shadow-lg border border-tertiary-light dark:border-tertiary mb-8">
                <div className="flex items-center space-x-4">
                    ...
                    <div>
                        <h2 className="text-3xl font-bold text-text-main-light dark:text-text-main">{groupInfo?.name}</h2>
