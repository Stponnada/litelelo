...
const UserSelectorModal: React.FC<UserSelectorModalProps> = ({ onClose, onConfirm, title = "Select Users", initialSelectedIds = [] }) => {
    const { user } = useAuth();
    ...
    useEffect(() => {
        const fetchUsers = async () => {
            setLoading(true);
            // Reuse the unified directory RPC to get all users
            const { data, error } = await supabase.rpc('get_unified_directory');
            if (error) {
                console.error("Error fetching users:", error);
            } else {
                // Filter out the current user and non-user profiles (if any)
    ...
    // Get full profile objects for selected users to display chips
    const selectedProfiles = availableUsers.filter(u => selectedIds.includes(u.id));
                ...
                <div className="px-8 py-6 bg-gray-50 dark:bg-gray-800/50 border-t border-gray-200 dark:border-gray-700">
                    <div className="flex items-center justify-end gap-3">
                        ...
                        <button
                            ...
                            className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-500 text-white font-semibold shadow-lg shadow-emerald-500/30 hover:shadow-xl hover:shadow-emerald-500/40 hover:scale-[1.02] active:scale-[0.98] transition-all duration-200"
                        >
                            Confirm Selection
                        </button>
                    </div>
