                ...
                <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-1">
                        {[...Array(5)].map((_, i) => (
                            <StarIcon
                                key={i}
                                className={`w-5 h-5 ${i < Math.floor(place.avg_rating)
...
const CampusDirectoryPage: React.FC = () => {
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <header className="mb-6 md:mb-10 relative overflow-hidden rounded-2xl bg-gradient-to-br from-purple-500/20 via-blue-500/10 to-transparent dark:from-purple-500/10 dark:via-blue-500/5 p-4 md:p-8 border border-purple-500/20">
                <div className="absolute top-0 right-0 w-32 h-32 md:w-64 md:h-64 bg-purple-500/10 rounded-full blur-3xl -mr-16 -mt-16 md:-mr-32 md:-mt-32"></div>
                <div className="absolute bottom-0 left-0 w-24 h-24 md:w-48 md:h-48 bg-blue-500/10 rounded-full blur-3xl -ml-12 -mb-12 md:-ml-24 md:-mb-24"></div>

                <div className="relative z-10">
                    <div className="flex flex-col md:flex-row md:items-center gap-4 md:gap-6 mb-6">
                        <div className="p-3 md:p-4 bg-gradient-to-br from-purple-500 to-blue-600 rounded-2xl shadow-lg w-fit">
                            <svg className="w-8 h-8 md:w-10 md:h-10 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
                            </svg>
                        </div>
                        <div className="flex-1">
                            <h1 className="text-3xl md:text-5xl font-extrabold text-text-main-light dark:text-text-main bg-gradient-to-r from-purple-500 to-blue-600 bg-clip-text text-transparent mb-2">
                                Campus Places
                            </h1>
                            <p className="text-sm md:text-lg text-text-secondary-light dark:text-text-secondary">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 md:gap-4">
                        <div className="bg-white/60 dark:bg-gray-800/60 backdrop-blur-sm rounded-xl p-3 md:p-4 border border-purple-500/20 flex items-center justify-between sm:block">
                            <div className="flex items-center gap-2 mb-0 sm:mb-1">
                                <svg className="w-5 h-5 text-purple-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                                </svg>
                                <p className="text-2xl md:text-3xl font-bold text-purple-500">{stats.totalPlaces}</p>
                            </div>
                            <p className="text-xs text-text-tertiary-light dark:text-text-tertiary font-bold uppercase tracking-wide">Places</p>
                        </div>
                        <div className="bg-white/60 dark:bg-gray-800/60 backdrop-blur-sm rounded-xl p-3 md:p-4 border border-blue-500/20 flex items-center justify-between sm:block">
                            <div className="flex items-center gap-2 mb-0 sm:mb-1">
                                <svg className="w-5 h-5 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 8h10M7 12h4m1 8l-4-4H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-3l-4 4z" />
                                </svg>
                                <p className="text-2xl md:text-3xl font-bold text-blue-500">{stats.totalReviews}</p>
                            </div>
                            <p className="text-xs text-text-tertiary-light dark:text-text-tertiary font-bold uppercase tracking-wide">Reviews</p>
                        </div>
                        <div className="bg-white/60 dark:bg-gray-800/60 backdrop-blur-sm rounded-xl p-3 md:p-4 border border-yellow-500/20 flex items-center justify-between sm:block">
                            <div className="flex items-center gap-2 mb-0 sm:mb-1">
                                <StarIcon className="w-5 h-5 text-yellow-500 fill-current" />
                                <p className="text-2xl md:text-3xl font-bold text-yellow-500">{stats.avgRating.toFixed(1)}</p>
                            </div>
            ...
            <div className="mb-8">
                <div className="flex items-center gap-3 mb-4">
                    <div className="h-1 w-8 md:w-12 bg-purple-500 rounded-full"></div>
                ...
                <div className="bg-gradient-to-br from-secondary-light to-tertiary-light/30 dark:from-secondary dark:to-tertiary/30 rounded-2xl p-4 md:p-6 shadow-lg border border-tertiary-light dark:border-tertiary">
                    ...
                    <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-thin scrollbar-thumb-purple-500 scrollbar-track-transparent">
                        {categories.map(category => (
                            <button
                                ...
                                className={`px-5 py-2.5 rounded-xl font-bold text-sm whitespace-nowrap transition-all duration-200 ${selectedCategory === category
                    ...
                    Object.entries(groupedPlaces).map(([location, placesInLocation]) => (
                        <div key={location} className="animate-fade-in">
                            <div className="flex items-center gap-4 mb-6">
                                ...
                                <div className="flex-1">
                                    <h2 className="text-2xl md:text-3xl font-bold text-text-main-light dark:text-text-main">{location}</h2>
