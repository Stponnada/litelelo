    ...
    <div className="bg-secondary-light dark:bg-secondary p-4 rounded-lg border border-tertiary-light dark:border-tertiary">
      ...
      {user?.id === review.user_id && (
        <div className="flex justify-end mt-2">
          <button onClick={() => onDelete(review.id)} className="text-xs text-red-500 hover:underline">
            Delete
          </button>
        </div>
      )}
...
const PlaceDetailPage: React.FC = () => {
  ...
  const fetchPlaceAndReviews = useCallback(async () => {
    ...
    try {
      ...
      const imagesPromise = supabase.rpc('get_images_for_place', { p_place_id: placeId });

      const [placeResult, reviewsResult, imagesResult] = await Promise.all([placePromise, reviewsPromise, imagesPromise]);
      ...
      <div className="bg-secondary-light dark:bg-secondary p-6 rounded-lg shadow-lg border border-tertiary-light dark:border-tertiary">
        ...
        <div className="relative w-full h-64 md:h-80 bg-tertiary-light dark:bg-tertiary rounded-md overflow-hidden mb-4">
          {images.length > 0 ? (
            ...
            <div className="flex items-center justify-center h-full text-text-tertiary-light dark:text-text-tertiary">No Image Available</div>
          )}
        </div>
        ...
        <form onSubmit={handleSubmitReview}>
          ...
          <div className="flex justify-end mt-4">
            <button
              ...
              className="bg-brand-green text-black font-bold py-2 px-6 rounded-full hover:bg-brand-green-darker transition-colors disabled:opacity-50"
            >
