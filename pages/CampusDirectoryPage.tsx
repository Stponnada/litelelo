...
import CampusImageUploadModal from '../components/CampusImageUploadModal'; // --- NEW: Import the modal ---
// --- NEW: Developer's user ID ---
...
return (
    <div className="group relative bg-gradient-to-br from-secondary-light to-primary-light dark:from-secondary dark:to-primary rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 border border-tertiary-light/30 dark:border-tertiary/30 hover:-translate-y-2">
        ...
        {/* --- NEW: Dev-only edit button --- */}
        {isDev && (
            <button 
                onClick={() => onEditImages(place)}
                className="absolute bottom-3 right-3 bg-blue-600 text-white text-xs font-bold px-3 py-1.5 rounded-lg shadow-lg opacity-0 group-hover:opacity-100 transition-opacity"
            >
                Edit Images
            </button>
        )}
...
return (
    <div className="max-w-7xl mx-auto">
        {/* --- NEW: Render the modal when a place is selected --- */}
        {editingPlace && (
            <CampusImageUploadModal 
                place={editingPlace}
                onClose={() => setEditingPlace(null)}
                onSuccess={fetchPlaces} // Refetch data on successful upload
            />
        )}
                ...
                Object.entries(groupedPlaces).map(([location, placesInLocation]) => (
                    <div key={location} className="animate-fade-in">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                            {placesInLocation.map(place => <PlaceCard key={place.id} place={place} onEditImages={setEditingPlace} isDev={user?.id === DEV_USER_ID} />)}
                        </div>
                    </div>
                ))
            ) : (
